...
scale_data <- scale_data %>%
  mutate(new_scored_response = case_when(
    !is.na(rev_scoring) & !is.na(numeric_response) ~ 6 - numeric_response,
    is.na(rev_scoring) & !is.na(numeric_response) ~ numeric_response,
    TRUE ~ NA_real_
  ))
...
comparison <- scale_data %>%
  filter(!is.na(new_scored_response) & !is.na(scored_response)) %>%
  mutate(matches = new_scored_response == scored_response) %>%
  filter(!matches)
